"""NRB — crawl configured index paths for PDF links."""
...
class NrbSource(BaseDocumentSource):
    async def discover(
        self,
        year_from: Optional[int] = None,
        year_to: Optional[int] = None,
    ) -> List[DocumentInfo]:
        return await self._discover_index_paths(year_from, year_to)

    async def _discover_index_paths(
        self,
        ...
        paths = self.config.get("index_paths") or [
            self.config.get("directives_path") or "/contents/directives",
        ]
                    ...
                    for pdf_url in extract_pdf_links(html, page_url):
                        ...
                        items.append(
                            DocumentInfo(
                                ...
                                url=pdf_url,
                                year=y,
                                metadata={"listing_page": page_url, "issuer": "NRB"},
                            )
                        )
                except Exception as e:
                    logger.warning("NRB discover failed for {}: {}", page_url, e)
        return items
