"""IRD — crawl configured index paths for PDF links."""
...
class IrdSource(BaseDocumentSource):
    async def discover(
        self,
        ...
        paths = self.config.get("index_paths") or [
            self.config.get("circulars_path") or "/",
        ]
                    ...
                    for pdf_url in extract_pdf_links(html, page_url):
                        ...
                        items.append(
                            DocumentInfo(
                                ...
                                url=pdf_url,
                                year=y,
                                metadata={"listing_page": page_url, "issuer": "IRD"},
                            )
                        )
                except Exception as e:
                    logger.warning("IRD discover failed for {}: {}", page_url, e)
        return items
